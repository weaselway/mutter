...
static UINT gfxredir_server_open(GfxRedirServerContext* context)
{
	...
	/* WTSVirtualChannelOpenEx takes LPSTR, but only reads the name. */
	priv->gfxredir_channel = WTSVirtualChannelOpenEx(
	    priv->SessionId, (LPSTR)GFXREDIR_DVC_CHANNEL_NAME, WTS_CHANNEL_OPTION_DYNAMIC);
	...
	if (priv->thread == NULL)
	{
		...
		if (!(priv->thread = CreateThread(NULL, 0, gfxredir_server_thread_func, (void*)context,
		                                  0, NULL)))
		{
			WLog_ERR(TAG, "CreateEvent failed!");
